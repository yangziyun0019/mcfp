...
def logger_init():
    ...
    logfile = './logs/robot-ctl-python.log'
...
class Auboi5Robot:
    ...
    def waypoint_to_pose_mm_deg(self, waypoint):
        """
        将 get_current_waypoint() 的返回（或类似包含 pos/ori 的 dict）转换为
        [x_mm, y_mm, z_mm, rx_deg, ry_deg, rz_deg]（位置 mm，姿态 度）。

        接受格式：
          - dict: {'pos': [x(m),y(m),z(m)], 'ori': [w,x,y,z]} 或包含键 x,y,z,w,x,y,z 等。
        返回 None 表示无法解析。
        """
        if waypoint is None:
            return None
        ...
        if isinstance(waypoint, dict):
            pos = waypoint.get('pos') or waypoint.get('position') or waypoint.get('cart')
            if pos is None and all(k in waypoint for k in ('x', 'y', 'z')):
        ...
        ori = None
        if isinstance(waypoint, dict):
            ori = waypoint.get('ori') or waypoint.get('orientation') or waypoint.get('quat')
            if ori is None and all(k in waypoint for k in ('w', 'x', 'y', 'z')):
                ori = [waypoint['w'], waypoint['x'], waypoint['y'], waypoint['z']]
        elif isinstance(waypoint, (list, tuple)) and len(waypoint) >= 7:
            # 可能形式为 [x,y,z,w,x,y,z]
            ori = list(waypoint[3:7])

        if ori is None or len(ori) != 4:
            return None

        # 将位置 m -> mm
        x_mm, y_mm, z_mm = [float(v) * 1000.0 for v in pos[:3]]

        # 四元数转欧拉（弧度），优先尝试库接口，否则使用纯 Python 计算
        rx_rad = ry_rad = rz_rad = None
        try:
            # 库接口（若实现返回弧度）
            rpy = self.quaternion_to_rpy(ori)
            if rpy and len(rpy) >= 3:
                rx_rad, ry_rad, rz_rad = map(float, rpy[:3])
        except Exception:
            rx_rad = ry_rad = rz_rad = None
