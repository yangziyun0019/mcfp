#!/bin/bash
# Script: run_pipeline_zyun.sh
# Purpose: Run the server-side reachability dataset pipeline while reusing the current ROS or virtual environment.
# Usage: bash tools/data_gen/scripts/runners/run_pipeline_zyun.sh [all|pos|orient|vis]
set -euo pipefail
...
# 服务器环境通常已经由外部 shell/虚拟环境准备好。
# 如确实需要显式 source 系统 ROS，再改成 true。
...
# 2) 激活 ROS 2 全局环境
if [ "${USE_SYSTEM_ROS_SETUP}" = true ]; then
  set +u
